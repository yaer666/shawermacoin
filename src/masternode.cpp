...
CMasternode::CollateralStatus CMasternode::CheckCollateral(const COutPoint& outpoint, const CKeyID& keyID, int& nHeightRet)
{
    ...
    if(coin.out.nValue != 585000000 * COIN) {
...
void CMasternode::Check(bool fForce)
{
    ...
    if(!fWaitForPing || fOurMasternode) {
        ...
        // part 1: expire based on shavermacoind ping
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    if (err == COLLATERAL_INVALID_AMOUNT) {
        LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 585000000 SWC, masternode=%s\n", outpoint.ToStringShort());
        nDos = 33;
        return false;
    }
