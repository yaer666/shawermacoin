...
// which are part of the Shavermacoin repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="shavermacoin-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /shavermacoin-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "shavermacoinpay-shavermacoin/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from shavermacoinpay-shavermacoin/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'shavermacoinpay-shavermacoin/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from shavermacoinpay-shavermacoin/develop"
            }
          }

          if (hasCache) {
            sh "cd /shavermacoin-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /shavermacoin-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /shavermacoin-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /shavermacoin-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /shavermacoin-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /shavermacoin-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /shavermacoin-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
